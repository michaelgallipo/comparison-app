...
class App extends Component {
  ...
  getweatherdata = (latitude,longitude, stateKey) => {
      fetch(`https://cors-anywhere.herokuapp.com/https://api.darksky.net/forecast/7f875b51f5e01333c06b1d559e43ce9a/${latitude},${longitude}`)
      .then(
        response => {
          ...
          response.json().then(data => {
            console.log(data);
            this.setState({...this.state, [stateKey]: data.daily.summary})
          });
        }
      )
      .catch(function(err) {
    ...
    return (
      <div className="App">
        ...
        <form onSubmit={this.coordinateSubmit}>
          ...
          <DataDisplay data={this.state.summary1}></DataDisplay>
          <DataDisplay data={this.state.summary2}></DataDisplay>
      </div>
    );
  }
}
